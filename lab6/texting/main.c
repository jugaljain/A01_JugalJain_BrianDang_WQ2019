...
volatile unsigned long intCounts[38];
volatile unsigned int numInts;
...
char textTranslate(int num, int count)
{
 ...
 //reset intCounts and numInts to 0's on account of false input
void clearInts(){
    while(numInts > 0){
        intCounts[numInts] = 0;
        numInts--;
...
int main()
{
    ...
    {   
        ...
        if(numInts == 0)
        {
            intCounts[numInts] = currTime;
            numInts++;
        }
        else{
            unsigned long length = currTime - prevTime;
            if(length > 100)
            {
                clearInts();
            }
                ...
                if(numInts < 38)
                {
                    intCounts[numInts] = length;
                    numInts++;
                }
                else{
                    pEnd = currTime;
                    MAP_GPIOIntDisable(GPIOA3_BASE, 0x40);
                }
            }
        }
        if(numInts == 38)
        {
            if(intCounts[1] >= 80 && intCounts[1] < 90)
            {
                num = decodeSignal();
            }
        }
